...
function ConnectionsPageContent() {
  const { data: connections, loading, error, mutate } = useApiData<Connection[]>(
    "connections", () => apiFetch("/api/connections")
  );
  ...
  function closeModal() {
    setShowModal(false); setNewConnId(null);
    setNewName(""); setQr(null); setModalError(null);
  }
    ...
    <div className="animate-fade-in">
        ...
        <div>
          ...
          {list.length > 0 && (
            <p className="mt-0.5 text-sm text-text-tertiary">
              {connectedCount} de {list.length} activa{connectedCount !== 1 ? "s" : ""}
            </p>
          )}
        </div>
        {list.length > 0 && (
          <button onClick={() => setShowModal(true)}
            className="flex items-center gap-2 rounded-xl bg-wa-green px-4 py-2 text-sm font-semibold text-text-inverse hover:bg-wa-green-dark transition-all hover:shadow-lg hover:shadow-wa-green/20">
      ...
      {!loading && !error && list.length > 0 && (
        <div className="space-y-2">
          {list.map((conn) => (
            <div key={conn.id}
              className="group relative flex items-center gap-4 rounded-2xl border border-border-primary bg-bg-secondary px-5 py-4 hover:border-border-secondary hover:bg-bg-elevated transition-all duration-150">

              {/* Avatar */}
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-bg-elevated text-base font-bold text-text-secondary">
              ...
              <a href={`/dashboard/connections/${conn.id}`} className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-text-tertiary font-mono">
                  {conn.phoneNumber ? `+${conn.phoneNumber}` : conn.id.slice(0, 16) + "…"}
                </p>
              </a>
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <a href={`/dashboard/connections/${conn.id}`}
                  className="rounded-lg p-2 text-text-tertiary hover:bg-bg-hover hover:text-text-primary transition-all" title="Ver">
                ...
                <button
                  ...
                  className="rounded-lg p-2 text-text-tertiary hover:bg-status-error-bg hover:text-status-error-text transition-all" title="Eliminar">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-sm rounded-3xl border border-border-secondary bg-bg-secondary p-6 shadow-2xl animate-scale-in"
            ...
            <p className="mt-1 text-center text-sm text-text-secondary">
              "{deleteTarget.name || "Conexión sin nombre"}" se eliminará permanentemente.
            </p>
            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-2">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-text-primary">
                  {qr ? "Escaneá el código QR" : newConnId ? "Preparando WhatsApp…" : "Nueva conexión"}
                </h2>
              </div>
            ...
            <div className="px-6 pb-6 pt-4">
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="ej. WhatsApp Principal" disabled={creating} autoFocus
              ...
              {qr && (
                <div className="flex flex-col items-center">
                  ...
                  <div className="mt-5 flex items-start gap-2 rounded-xl bg-bg-elevated px-4 py-3 text-sm text-text-secondary max-w-xs text-center">
                  ...
                  {modalError && (
                    <p className="mt-3 text-xs text-status-error-text">{modalError}</p>
                  )}
                </div>
              )}
            </div>
